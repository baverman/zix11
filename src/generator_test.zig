...
const toy = @import("testgen/core.zig");
...
test "DrawableHolder encode and decode use the xidunion shape" {
    try std.testing.expectEqual(@as(u32, 0), @intFromEnum(toy.Window.None));
...
test "extension can use scoped core enum beside local enum with same name" {
    ...
    try std.testing.expectEqual(toy.Mode.On, decoded.core_mode);
    try std.testing.expectEqual(foo.Mode.Local, decoded.mode);
}

test "extension reply decodes byte field from body payload" {
    var packet = [_]u8{ 7, 0x34, 0x12 };
    var reader: std.Io.Reader = .fixed(&packet);
    const reply = try foo.FooPing.Reply.decode(&reader, .{ .byte_slot = 99, .length = 0 });
...
test "UseByteField reply decode accepts byte1 from caller" {
    ...
    var reader: std.Io.Reader = .fixed(&packet);
    const reply = try toy.UseByteField.Reply.decode(&reader, .{ .byte_slot = 7, .length = 0 });
...
test "UseBytePad reply decode ignores byte1 and reads payload" {
    ...
    var reader: std.Io.Reader = .fixed(&packet);
    const reply = try toy.UseBytePad.Reply.decode(&reader, .{ .byte_slot = 99, .length = 0 });
...
test "UseCard32 reply decode ignores byte1 and reads payload" {
    const packet = makePacket(&tmp, .{&[_]u32{0x01020304}});
    var reader: std.Io.Reader = .fixed(packet);
    const reply = try toy.UseCard32.Reply.decode(&reader, .{ .byte_slot = 17, .length = 0 });
