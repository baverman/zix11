...
pub fn get(conn: *Connection, name: []const u8) Connection.RequestError!x.Atom {
...
pub fn getOrCreate(conn: *Connection, name: []const u8) Connection.RequestError!x.Atom {
    const reply = try conn.request(x.InternAtom, .{
...
pub fn getAll(comptime T: type, conn: *Connection) Connection.RequestError!T {
