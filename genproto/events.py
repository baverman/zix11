...
from . import xcbxml
from .common import BaseType, Emit, Field, Size, emit_decl_items, emit_deinit_items, expr_refs, items_size
...
def emit_events_module(modules: Sequence[Module], ext_import: str = '../_ext.zig') -> str:
    ...
    for module in modules:
        ...
        with emit.block():
            ...
            emit(
                f'.decode_xge = {"%s.decodeXgeEvent" % module.name if xge_numbers else "null"},'
            )
        emit('};')
        emit()
    emit('pub fn eventSpec(extension: extensions.Extension) ?*const ExtensionEventSpec {')
    with emit.block():
        modules_with_events = [module for module in modules if module.events]
        if modules_with_events:
            emit('return switch (extension) {')
            with emit.block():
                for module in modules_with_events:
                    emit(
                        f'.{module.extension_enum_name()} => &{module.name}_event_spec,'
                    )
            emit('};')
        else:
            emit('_ = extension;')
